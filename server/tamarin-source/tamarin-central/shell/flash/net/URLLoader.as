...
public class URLLoader extends EventDispatcher
{
    ...
    */
    ...
    {
       // If it is a file we can access locally, return that instead.
       if(File.exists(request.url))
       {
          // Great, so set a delay and service the request immediately.
          setTimeout(function():void { _HandleImmediateLoad(request.url); }, 10);
          return;
       }
    ...
    private function _HandleImmediateLoad(filename:String):void
    {
       if(!File.exists(filename))
          throw new Error("Somebody deleted a file from underneath us!");
       
       var data:ByteArray = File.fileToByteArray(filename, true);
       if(!data)
          throw new Error("Failed to read from file '" + filename + "'!");
       
       // Fire some events so it is reported as complete.
       _bytes = data;
       handleComplete(null);
    }
