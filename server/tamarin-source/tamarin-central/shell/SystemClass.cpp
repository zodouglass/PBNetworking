...
namespace avmshell
{
   ...
   bool SystemClass::doHeartbeat()
   {
      // If none, we can exist.
      if(!heartbeatCallback)
         return false;

      // Make sure it's a function!
      if(!heartbeatCallback->core()->isFunction(heartbeatCallback->atom()))
         return false;

      // Call it!
      Atom r = heartbeatCallback->call_this(0);

      // Cast to a bool for return values.
      return heartbeatCallback->core()->boolean(r) != 0;
   }
}
