...
namespace avmshell
{
	...
	void Shell::usage()
	{
      ...
      printf("          [--daemon]       Spin off in a background process; suitable for a UNIX daemon/server.\n");
      printf("          [-u userid]      Run as specified user.\n");
      printf("          [--pidfile file] Write process ID to specified file.\n");
      printf("          [--chroot path]  Change root to specified path before starting ABC execution.\n");
...
#define strcpy(_str, _conststr)		_tcscpy(_str, _conststr)
...
#endif
	{
		...
		TRY(this, kCatchAction_ReportAsError)
		{
         ...
         const char *chroot_path = NULL;
			...
			for (int i=1; i<argc && endFilenamePos == -1; i++) {
				...
				if (arg[0] == '-') 
				{
                   ...
                   switch_to_user = atoi(argv[++i]);
               }
               else if(!strcmp(arg, "--pidfile")) {
                   pid_file = argv[++i];
               }
               else if(!strcmp(arg, "--chroot")) {
                   chroot_path = argv[++i];
               }
	                #ifdef DEBUGGER
         ...
         // Do daemonization/chroot/setuser.
         if(do_daemon)
         {
#ifdef WIN32
            printf("Daemon mode not supported for Win32.\n");
            exit(1);
#else
         ...
         if(pid_file)
         {
            printf("Writing PID to '%s'\n", pid_file);
            FILE *pidHandle = fopen(pid_file, "w");
            fprintf(pidHandle, "%d", getpid());
            fclose(pidHandle);
         }
         ...
         {
#ifdef WIN32
            printf("Cannot chroot in Win32. Only chdir'ing instead.\n");
#else
            printf("chroot'ing to '%s'\n", chroot_path);
            int chroot_res = chroot(chroot_path);
            if(chroot_res != 0) 
            {
               printf("   - Failed with %d! Aborting.", chroot_res);
               exit(1);
            }
#endif

            printf("chdir'ing to chroot '%s'\n", chroot_path);
            chdir(chroot_path);
         }
         ...
         {
#ifdef WIN32
            printf("Cannot change UID, setuid doesn't exist under Win32.\n");
#else
            printf("Switching to UID '%d'.\n", switch_to_user);
            setuid(switch_to_user);
#endif
         }
         ...
         // Run the heartbeat till it says to quit.
         while (systemClass->doHeartbeat()) 
         {
